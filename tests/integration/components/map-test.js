...
module('Integration | Component | map', function (hooks) {
  ...
  test('it renders a map image for the specified parameters', async function () {
  ...
  test('it updates the `src` attribute when the arguments change', async function () {
    this.setProperties({
      lat: 37.7797,
      lng: -122.4194,
      zoom: 10,
    ...
    assert.ok(
      img.src.includes('-122.4194,37.7797,10'),
      'the src should include the lng,lat,zoom parameter',
    );
    ...
    assert.ok(
      img.src.includes('-122.4194,37.7797,12'),
      'the src should include the lng,lat,zoom parameter',
    );
  ...
  test('the default alt attribute can be overridden', async function () {
    await render(hbs`<Map
      @lat="37.7797"
      @lng="-122.4184"
      @zoom="10"
  ...
  test('the src, width, and height attributes cannot be overridden', async function () {
    await render(hbs`<Map
      @lat="37.7797"
      @lng="-122.4184"
      @zoom="10"
